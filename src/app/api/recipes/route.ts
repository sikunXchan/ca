...
export async function POST(req: Request) {
  try {
    ...
    const prompt = `あなたは経験豊富なプロシェフです。以下の在庫食材を使い、家庭で再現できる「本当においしい」レシピを複数提案してください。
【現在の在庫食材】
${ingredients.join(', ')}
...
4. 【質のポイント】各レシピは以下の観点で設計してください：食感のコントラスト（サクサク×とろとろ、シャキシャキ×柔らかなど）、旨み・酸味・甘み・塩味のバランス、見た目の彩り。料理の完成形が「食べたい」と思えるレベルを目指してください。
5. 【手順の具体性】各ステップには必ず「中火で3分」「表面がこんがりきつね色になるまで」「沸騰したら弱火に落として」など、温度・火加減・時間・視覚的なキューを含めてください。「炒める」「煮る」だけの曖昧な手順は禁止です。
