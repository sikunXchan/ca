...
2. 味付けは「ぼやけない・しっかりした美味しさ」を最優先し、すべての調味料の分量を「大さじ・小さじ・g」など具体的な数値で必ず明記してください（「適量」「少々」は仕上げの塩・こしょう等のみ許可）。
3. 【質のポイント】食感のコントラスト（サクサク×とろとろなど）、旨み・酸味・甘み・塩味のバランスを意識して設計してください。完成形が「食べたい」と思えるレベルを目指してください。
4. 【手順の具体性】各ステップには「中火で3分」「表面がこんがりきつね色になるまで」など、温度・火加減・時間・視覚的なキューを必ず含めてください。「炒める」「煮る」だけの曖昧な手順は禁止です。
5. 以下のJSON構造で返してください。これ以外のテキストは一切含めないでください。
{
  "title": "リメイク後の料理名",
  "time": "調理時間目安（例：15分）",
  "genre": "和食",
